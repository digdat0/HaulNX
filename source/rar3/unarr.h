...
/* HaulNX note: upstream unarr also declares ar_open_zip_archive/ar_open_7z_archive/
 * ar_open_tar_archive here. HaulNX only ported rar/ (libarchive already covers
 * zip/7z/tar fine -- this fallback exists solely for the RAR filter gap), so
 * those declarations are omitted rather than left dangling with no matching .c. */
